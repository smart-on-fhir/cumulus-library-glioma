...
class AgeAtDiagnosisMention(SpanAugmentedMention):
    age_years: Optional[float] = Field(
        None,
        description="Age at glioma diagnosis in years, if explicitly stated."
    )
...
class GliomaDiagnosisAnnotation(BaseModel):
    ...
    tumor_size: TumorSizeMassEffectMention
    topography: TopographyMention
    morphology: MorphologyMention
    behavior: BehaviorMention
    grade: GradeMention
    nf1_status: NF1StatusMention
